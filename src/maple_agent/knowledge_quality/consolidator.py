...
def to_generic_packet(packet: dict) -> dict:
    ...
    if "npcs" in packet or "monsters" in packet or "items" in packet:
        return packet
    generic: dict = {"maps": [], "npcs": [], "monsters": [], "items": [], "relations": []}
...
def _external_id(entity) -> str:
    for attr in ("map_id", "npc_id", "monster_id", "item_id"):
        ...
        entity_counts = {
            ...
            "quest": len(
                [
                    entity
                    for entity in packet.get("entities", [])
                    if entity.get("knowledge_type") == "QUEST"
                ]
            ),
        }
        relation_counts = {
