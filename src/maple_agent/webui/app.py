...
def create_app(
    *,
    ...
    @app.get("/api/spatial-world/state")
    async def api_spatial_world_state():
        if spatial_world is None:
            return {"enabled": False}
        return {"enabled": True, **spatial_world}

    @app.get("/api/navigation/state")
    async def api_navigation_state():
    ...
    @app.get("/api/action-proposal/state")
    async def api_action_proposal_state():
